...
export function formatClassifySummary(output: ClassificationOutput): string {
  const answer = output.answer_markdown ? `사용자에게는 아래 답변을 우선 전달하세요.\n\n${output.answer_markdown}` : "";
  if (output.result_type === "emergency_redirect") {
    return answer || `${output.issue.label_ko}: ${output.safety.emergency_redirect?.message ?? output.user_messages.summary}`;
  }
  if (output.result_type === "needs_clarification") {
    return answer || (output.user_messages.clarifying_question ?? output.user_messages.summary);
  }
  if (output.result_type === "out_of_scope") {
    return answer || `${output.user_messages.summary} ${output.draft_policy.reason}`;
  }
